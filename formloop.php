...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["position"])) {
        ...
        $positions = $_POST["position"]; // Get the array of positions
        ...
        // Assign the positions array to the $position variable
        $position = $positions;
    }
    ...
    if (empty($_POST["count"])) {
        ...
        $counts = $_POST["count"]; // Get the array of counts
        ...
        // Assign the counts array to the $count variable
        $count = $counts;
    }
    ...
    if (empty($_POST["location"])) {
        ...
        $locations = $_POST["location"]; // Get the array of counts
    ...
    if (empty($companyname_err) && empty($fullname_err) && empty($mobilenum_err) && empty($companyadd_err) && empty($email_err) && empty($position_err) && empty($count_err) && empty($location_err) && empty($remarks_err)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            $param_position = $position[$i];
            $param_count = $count[$i];
            $param_location = $location[$i];

            // Attempt to execute the prepared statement
            if (mysqli_stmt_execute($stmt)) {
              // Show success alert and redirect to login page
              echo "<script>alert('Form sent!'); location.href='services-tg.html';</script>";
              exit();
          } else {
              echo "Oops! Something went wrong. Please try again later.";
          }
...
<body>
          ...
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
             ...
             <div class="row">
                ...
                <div class="col-md-12">
                  <h4>Remarks</h4>
                  <div class="form-group">
              <textarea name="remarks" class="form-control form-control-body <?php echo (!empty($remarks_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $remarks; ?>" id="exampleFormControlTextarea1" placeholder="Please add Remarks, Leave as N/A if none."></textarea>
  ...
  <script>
    var rowCounter = 0; // Counter for dynamically added rows
...
function addRow() {
  ...
  newRow.innerHTML = `
    ...
    <div class="col-md-3">
      <div class="form-group">
        ...
        <input type="number" name="count[]" class="form-control" placeholder="Enter count" min="0" required>
      </div>
    </div>
    <div class="col-md-4">
      <div class="form-group">
  ...
  // Get the container
  var container = document.getElementById('dynamicRowsContainer');

  // Insert the new row above the "Add" button
  var addButtonRow = container.lastElementChild;
  container.insertBefore(newRow, addButtonRow);

  // Increment the row counter
  rowCounter++;

  // Update the IDs of the elements in the new row to ensure uniqueness
  updateElementIds(newRow);
}
...
function updateElementIds(row) {
  // Get all select elements within the row
  var selects = row.getElementsByTagName('select');
  // Get all input elements within the row
  var inputs = row.getElementsByTagName('input');

  // Update the IDs of select elements
  for (var i = 0; i < selects.length; i++) {
    var select = selects[i];
    select.id += '_' + rowCounter; // Append the row counter to the ID
  }

  // Update the IDs of input elements
  for (var j = 0; j < inputs.length; j++) {
    var input = inputs[j];
    input.id += '_' + rowCounter; // Append the row counter to the ID
  }
}
</script>

...
if (isset($_GET["key"]) && isset($_GET["email"]) && isset($_GET["action"]) && ($_GET["action"]=="reset") && !isset($_POST["action"])){
  ...
  if ($row==""){
    ...
    if ($expDate >= $curDate){
      ...
      <p>The link is expired. You are trying to use the expired link which is valid only 24 hours (1 day after request).<br /><br /></p>";
    }
  }
...
if(isset($_POST["email"]) && isset($_POST["action"]) && ($_POST["action"]=="update")){
  ...
  if($error!=""){
    ...
    <p><a href="http://172.29.105.55/~teamglobal/login.php">Click here</a> to Login.</p></div><br />';
